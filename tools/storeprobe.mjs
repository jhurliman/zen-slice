...
check('the veil shows the price and the restore line, no App Store link', V && V.up && V.buy === 'unlock the full game · $2.99' && V.restore === true && V.link === null, JSON.stringify(V));
...
G = await page.evaluate(reachGate);
V = await page.evaluate(veil);
check('localized price string is shown verbatim', V && V.buy === 'unlock the full game · 2,99 €', JSON.stringify(V));
await page.evaluate(() => window.__tap('.zs-demo-buy'));
await page.waitForTimeout(100);
V = await page.evaluate(veil);
R = await page.evaluate(() => { const S = window.ZS.ctx.store; return { entitled: S.entitled, busy: S.busy }; });
check('a cancelled purchase leaves the veil up with the button restored', V && V.up && V.buy === 'unlock the full game · 2,99 €' && R.entitled === false && R.busy === false, JSON.stringify({ V, R }));
...
check('the panel carries "unlock the full game" while the day is not owned', R.panel && R.row, JSON.stringify(R));
